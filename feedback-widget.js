...
(function () {
  ...
  function showCommentForm(xPct, yPct, cx, cy, elementContext) {
    closeAllPopups();
    ...
    captureScreenshot(xPct, yPct).then(data => {
      screenshotData = data;
  ...
  function boot() {
    ...
    document.getElementById('fb-overlay').addEventListener('click', (e) => {
      ...
      const xPct    = parseFloat(((e.clientX / window.innerWidth)  * 100).toFixed(3));
      const yPct    = parseFloat(((e.clientY / window.innerHeight) * 100).toFixed(3));
      const context = captureElementContext(e.clientX, e.clientY);
      exitAnnotationMode();   // exit mode before screenshot so overlay doesn't appear
      showCommentForm(xPct, yPct, e.clientX, e.clientY, context);
    });
